...
const migrationPath = "prisma/baselines/p22c3a-production-authority-schema-only.sql";
...
const manifest = JSON.parse(readFileSync("docs/P22C3C_PRODUCTION_DDL_GATE_MANIFEST.json", "utf8"));
...
assert.equal(packageJson.scripts["test:p22c3c-production-ddl-gate"], "node scripts/p22c3c-production-ddl-gate.test.mjs");
...
assert.equal((snapshot.match(/WHERE brand_key = 'SALORA'/gu) ?? []).length, 2);
assert.ok((preflight.match(/brand_key = 'SALORA'/gu) ?? []).length >= 6);
assert.match(preflight, /AND category\.brand_key = 'SALORA'[\s\S]*WHERE product\.brand_key = 'SALORA'[\s\S]*AND category\.id IS NULL/u);
assert.match(preflight, /FROM public\.catalog_products[\s\S]*WHERE brand_key = 'SALORA'[\s\S]*GROUP BY slug/u);
assert.match(preflight, /FROM public\.product_categories[\s\S]*WHERE brand_key = 'SALORA'[\s\S]*GROUP BY slug/u);
assert.equal((postApply.match(/WHERE brand_key = 'SALORA'/gu) ?? []).length, 2);
assert.deepEqual(manifest.catalogScope, { brandKey: "SALORA", productAuthorityOnly: true, categoryAuthorityOnly: true, fullCatalogRowsExcluded: true });
assert.equal(manifest.correction.phase, "P22C-3C-A");
for (const artifact of readOnlyArtifacts) {
  ...
  for (const forbidden of [/^\s*CREATE\s+(?:TABLE|TYPE|INDEX|POLICY|FUNCTION|TRIGGER|EXTENSION)\b/imu, /^\s*ALTER\s+TABLE\b/imu, /^\s*DROP\s+/imu, /^\s*INSERT\s+INTO\b/imu, /^\s*UPDATE\s+(?!OF\b)/imu, /^\s*DELETE\s+FROM\b/imu, /^\s*TRUNCATE\b/imu, /^\s*COPY\s+/imu, /^\s*MERGE\s+INTO\b/imu]) assert.doesNotMatch(artifact, forbidden);
}
...
for (const marker of ["PREPARATION ONLY", "separate final approval", "MENU_AUTHORITY_MODE=compatibility", "117 / 104 ACTIVE / 13 DRAFT / 16 categories", "Snapshot", "Preflight", "Rollback", "P22C-3D", "P22C-3E", "No Production connection was made"]) assert.ok(documentation.includes(marker), `Missing documentation marker: ${marker}`);

console.log("SALORA P22C-3C historical DDL gate verified:");
console.log("- certified baseline SHA is unchanged and archived outside prisma/migrations");
console.log("- historical provider identity is explicitly non-routable");
console.log("- snapshot, preflight, and post-apply probes remain read only");
console.log("- Production execution remains disabled pending a dedicated current Production resource");
