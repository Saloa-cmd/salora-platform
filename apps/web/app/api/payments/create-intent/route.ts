import { createPaymentIntentSchema, createRevenuePaymentIntent, redactPaymentError } from "@salora/backend";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const parsed = await parseJson(request, createPaymentIntentSchema);
    if (!parsed.success) return responseError("Invalid payment intent payload.", requestId, 400);

    const result = await createRevenuePaymentIntent(parsed.data);
