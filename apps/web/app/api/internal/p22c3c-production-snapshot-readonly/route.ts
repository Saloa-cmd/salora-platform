...
const GATE_TOKEN_SHA256 = "278dc1c39207779efab5490128a06bb0bbc3857e5a7de6a5c343ad771fff1fb2";
const GATE_EXPIRES_AT = "2026-08-12T19:00:00+04:00";
...
function respond(body: Record<string, unknown>, status: number) {
  return Response.json(body, { status, headers: {
    ...
    "X-Salora-P22C3C-Gate": "PRODUCTION_SNAPSHOT_READ_ONLY_REDACTED"
  }});
}
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const db = identity[0];
    if (!db || db.default_transaction_read_only !== "on" ||
        db.server_version_num < 170000 || db.in_recovery)
      return fail("READ_ONLY_DATABASE_IDENTITY_GUARD_FAILED", 409);

    try {
      await prisma.$transaction(async (tx) => {
        const settings = await tx.$queryRawUnsafe<Array<{
          transaction_read_only: string; default_transaction_read_only: string;
        }>>(`SELECT current_setting('transaction_read_only') AS transaction_read_only,
          current_setting('default_transaction_read_only') AS default_transaction_read_only;`);
        if (settings[0]?.transaction_read_only !== "on" ||
            settings[0]?.default_transaction_read_only !== "on")
          throw new Error("SNAPSHOT_READ_ONLY_GUARD_FAILED");
    ...
    consumed = true;
    return respond({
      phase: "P22C-3C", result: "PASS", operation: "PRODUCTION_SNAPSHOT_READ_ONLY_REDACTED",
      productionProjectRef: P22C3C_PRODUCTION_PROJECT_REF,
      deploymentGitCommitSha: process.env.VERCEL_GIT_COMMIT_SHA ?? null,
      databaseIdentity: { projectRefVerified: true,
        defaultTransactionReadOnly: db.default_transaction_read_only,
        serverVersionMajorAtLeast17: db.server_version_num >= 170000, inRecovery: db.in_recovery },
      snapshot: { phase: snapshot.phase, mode: snapshot.mode,
      ...
      safety: { evidenceRedacted: true, fingerprintsReturned: false, slugsReturned: false,
        databaseUrlExposed: false, databaseUrlLogged: false, snapshotTransactionRolledBack: true,
