...
function PillList({ items }: { items: string[] }) {
  return (
    <div className="flex flex-wrap gap-2">
      {items.map((item) => (
        <span key={item} className="rounded-full border border-white/10 bg-white/5 px-3 py-2 text-xs font-semibold text-[var(--muted)]">{item}</span>
      ))}
    </div>
  );
}
...
function BackendActivationCard({ title, items }: { title: string; items: string[] }) {
  return (
    <DashboardCard title={title} eyebrow="No-code workspace">
      <p className="mb-4 text-sm leading-6 text-[var(--muted)]">This workspace is modeled in the Control Tower and ready for persistent backend activation. It is intentionally not marked live until the domain API, audit trail, and rollback contract exist.</p>
      ...
      <section className="rounded-xl border border-[var(--border-gold)] bg-[var(--gold)]/10 p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-start gap-4">
            ...
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[var(--gold-soft)]">Control Tower Section</p>
              <h2 className="mt-1 text-2xl font-semibold text-[var(--cream)]">{section.label}</h2>
              <p className="mt-2 max-w-4xl text-sm leading-6 text-[var(--muted)]">{section.description}</p>
            </div>
          </div>
        </div>
      </section>

      <DashboardSection title="Capabilities" description="Live capabilities use existing SALORA APIs. Pending capabilities are modeled without claiming production readiness.">
        <DashboardGrid columns="two">
          {section.capabilities.map((capability) => <CapabilityCard key={capability.title} capability={capability} />)}
        </DashboardGrid>
      </DashboardSection>

      <DashboardSection title="No-Code Workspace" description="Operator-first controls for the selected business capability. Live write actions remain protected by existing RBAC permissions.">
