...
function DomainWorkspace({ sectionId }: { sectionId: ControlTowerSectionId }) {
  ...
  if (sectionId === "customers") return <SectionTabs label={t("أدوات العملاء", "Customer tools")} tabs={[{ id: "harmony", label: t("Harmony Rewards", "Harmony Rewards"), content: <HarmonyCustomerWorkspace /> }, { id: "customers", label: t("العملاء", "Customers"), content: <DashboardView kind="customers" /> }, { id: "loyalty", label: t("الولاء", "Loyalty"), content: <LoyaltyActionPanel /> }]} />;
  ...
  if (sectionId === "operations") return <SectionTabs label={t("أدوات التشغيل", "Operations tools")} tabs={[{ id: "status", label: t("الحالة", "Status"), content: <DashboardView kind="operations" /> }, { id: "whatsapp", label: t("واتساب", "WhatsApp"), content: <WhatsAppCommandCenter /> }, { id: "health", label: t("صحة النظام", "System health"), content: <OperationalGovernanceCenter /> }]} />;
  return <SectionTabs label={t("أدوات الإعدادات", "Settings tools")} tabs={[{ id: "configuration", label: t("الإعدادات", "Configuration"), content: <RuntimeConfigActionPanel /> }, { id: "health", label: t("الصحة والتدقيق", "Health & audit"), content: <OperationalGovernanceCenter /> }]} />;
}

export function ControlTowerView({ sectionId }: { sectionId?: string }) {
  const { tr } = useControlTowerLocale(); const section = findControlTowerSection(sectionId); const isOverview = section.id === "overview";
  return <div className="space-y-6"><header className="flex items-start gap-3 sm:gap-4"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl border border-[var(--border-gold)] bg-[var(--gold)]/10 text-[var(--gold-soft)]"><SaloraIcon name={section.icon} className="h-5 w-5" /></span><div className="min-w-0"><p className="text-xs font-semibold text-[var(--gold-soft)]">SALORA</p><h1 className="mt-1 text-2xl font-semibold tracking-tight sm:text-3xl">{isOverview ? tr("Today at SALORA") : tr(section.label)}</h1><p className="mt-1 max-w-2xl text-sm leading-6 text-[var(--muted)]">{tr(section.description)}</p></div></header><DomainWorkspace sectionId={section.id} /></div>;
}
