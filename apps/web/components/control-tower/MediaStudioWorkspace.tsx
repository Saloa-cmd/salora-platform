...
export function MediaStudioWorkspace() {
  ...
  useEffect(() => {
    let active = true;
    void Promise.all([
      controlTowerGet<Product[]>("/api/control-tower/simple-launch/products?limit=100&offset=0"),
      controlTowerGet<Product[]>("/api/control-tower/simple-launch/products?limit=100&offset=100")
    ]).then(([a, b]) => {
      if (!active) return;
  ...
  function toggleAsset(assetId: string) { setScenes((current) => current.map((scene, index) => index === 0 ? { ...scene, assetIds: scene.assetIds.includes(assetId) ? scene.assetIds.filter((id) => id !== assetId) : [...scene.assetIds, assetId] } : scene)); }
