...
export function HarmonyCustomerWorkspace(){
 ...
 useEffect(()=>{if(!selected)return;void controlTowerGet<any>(`/api/control-tower/harmony?customerId=${encodeURIComponent(selected.customerId)}`).then(r=>{if(r.status==="success")setDetail(r.data);});},[selected]);
