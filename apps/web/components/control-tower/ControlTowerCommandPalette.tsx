...
export function ControlTowerCommandPalette({ visibleSections, open, onOpenChange }: { visibleSections: ControlTowerSectionId[]; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const { tr: translate } = useControlTowerLocale();
  const tr = (value: string | undefined) => translate(value ?? "");
    ...
    <div className="fixed inset-0 z-[80] grid place-items-start bg-black/65 px-4 pt-[12vh] backdrop-blur-sm" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onOpenChange(false); }}>
      <section role="dialog" aria-modal="true" aria-label={tr("Command palette")} className="mx-auto w-full max-w-2xl overflow-hidden rounded-2xl border border-[var(--border-gold)] bg-[var(--surface)] shadow-2xl">
        ...
        <div className="max-h-[55vh] overflow-y-auto p-2">
          ...
          {query.trim().length >= 2 ? <div className="mt-2 border-t border-white/10 pt-2"><p className="px-3 py-2 text-[10px] font-semibold uppercase tracking-[0.2em] text-[var(--muted)]">{tr("Global results")}</p>{resolvedQuery !== query.trim() ? <p className="px-4 py-4 text-sm text-[var(--muted)]">{tr("Searching permitted data…")}</p> : entities.length ? entities.map((result) => result.type === "product" ? <div key={`product-${result.id}`} className="rounded-xl px-3 py-3 hover:bg-white/[0.06]"><button type="button" onClick={() => { router.push(result.href); onOpenChange(false); }} className="flex w-full items-center gap-3 text-start"><span className="grid h-9 w-9 place-items-center rounded-lg bg-white/[0.05] text-[var(--gold-soft)]"><SaloraIcon name="menu" className="h-4 w-4" /></span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold">{result.title}</span><span className="mt-1 block truncate text-xs text-[var(--muted)]">{result.detail}</span></span></button><div className="mt-3 flex flex-wrap gap-2 ps-12">{[["price", "Price"], ["media", "Media"], ["availability", "Availability"], ["preview", "Preview"]].map(([action, label]) => <button key={action} type="button" onClick={() => { if (action === "preview") window.open(`/menu?product=${encodeURIComponent(result.id)}`, "_blank", "noopener,noreferrer"); else router.push(`${result.href}&action=${action}`); onOpenChange(false); }} className="min-h-9 rounded-lg border border-white/10 px-3 text-[11px] font-semibold text-[var(--muted)] hover:border-[var(--border-gold)] hover:text-[var(--cream)]">{tr(label)}</button>)}</div></div> : <button key={`${result.type}-${result.id}`} type="button" onClick={() => { router.push(result.href); onOpenChange(false); }} className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-start hover:bg-white/[0.06]"><span className="grid h-9 w-9 place-items-center rounded-lg bg-white/[0.05] text-[var(--gold-soft)]"><SaloraIcon name={result.type === "order" ? "orders" : result.type === "customer" ? "user" : "menu"} className="h-4 w-4" /></span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold">{result.title}</span><span className="mt-1 block truncate text-xs text-[var(--muted)]">{result.type}{result.detail ? ` · ${result.detail}` : ""}</span></span></button>) : <p className="px-4 py-4 text-sm text-[var(--muted)]">{tr("No permitted entity matches your search.")}</p>}</div> : null}
        </div>
