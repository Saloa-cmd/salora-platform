-- SALORA P22C-3A — Production-specific Menu Authority schema only
-- Repository artifact only. This file is NOT approved for Production execution.
-- Built for the certified legacy Production baseline:
--   117 products / 104 ACTIVE / 13 DRAFT / 16 categories.
--
-- Guarantees:
--   * no INSERT, UPDATE, DELETE, MERGE, COPY, TRUNCATE, seed, or publication data
--   * no mutation of catalog_products or product_categories
--   * no staging-only metadata tables
--   * compatibility mode remains an application/runtime concern and is not changed here
--   * authority objects must be fully absent before execution
--
...
BEGIN
  IF to_regclass('public.catalog_products') IS NULL THEN
    missing_prerequisites := array_append(
  ...
  IF to_regtype('public."RoleName"') IS NULL THEN
    missing_prerequisites := array_append(
      missing_prerequisites,
      'public."RoleName"'
    );
  ...
  IF to_regtype('public."ProductStatus"') IS NULL THEN
    missing_prerequisites := array_append(
      missing_prerequisites,
  ...
  IF to_regprocedure('public.salora_jwt_roles()') IS NULL THEN
    missing_prerequisites := array_append(
      missing_prerequisites,
  ...
  IF to_regprocedure('public.salora_is_staff()') IS NULL THEN
    missing_prerequisites := array_append(
      missing_prerequisites,
  ...
  IF to_regprocedure('public.salora_is_manager()') IS NULL THEN
    missing_prerequisites := array_append(
      missing_prerequisites,
  ...
  IF to_regprocedure('public.salora_is_admin()') IS NULL THEN
    missing_prerequisites := array_append(
      missing_prerequisites,
  ...
  IF cardinality(missing_prerequisites) > 0 THEN
    RAISE EXCEPTION
      'P22C-3A prerequisite failure: %',
      array_to_string(missing_prerequisites, ', ');
  END IF;

  IF cardinality(existing_authority_objects) > 0 THEN
    RAISE EXCEPTION
      'P22C-3A authority schema is not fully absent: %',
      array_to_string(existing_authority_objects, ', ');
  END IF;
END;
$p22c3a_preflight$;

-- SALORA P21 â€” Menu Collections Domain Foundation
...
USING (
  (
    ...
    AND EXISTS (
      ...
      WHERE membership.product_id = product_nutrition_profiles.product_id
...
USING (
  (
    ...
    AND EXISTS (
      ...
      JOIN public.menu_collections collection ON collection.id = membership.collection_id
      JOIN public.catalog_products product ON product.id = membership.product_id
      WHERE membership.product_id = product_allergen_profiles.product_id
        AND membership.archived_at IS NULL
        AND collection.status = 'PUBLISHED'
        AND collection.archived_at IS NULL
        AND product.status = 'ACTIVE'
...
-- P21A final security and performance state, folded into P22C-3A.
-- SALORA P21A â€” Menu Collections Security & Performance Hardening
